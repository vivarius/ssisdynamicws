                ...
                using (var wsdlHandler = new WSDLHandler(new Uri(EvaluateExpression(ServiceUrl, variableDispenser).ToString())))
                {
                    ...
                    if (result != null)
                    {
                        ...
                        _vars[val.Substring(0, val.Length - 1)].Value =
                            wsdlHandler.GetServiceMethods(EvaluateExpression(Service, variableDispenser).ToString()).
                                Select(m => m.Equals(EvaluateExpression(WebMethod, variableDispenser).ToString())).FirstOrDefault();
                    }
                }
            }
            catch (Exception ex)
